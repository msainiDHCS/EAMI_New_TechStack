...
{
    public partial class NewPaymentSubmissionOp : Form
    {
        ...
        public NewPaymentSubmissionOp()
        {
            InitializeComponent();
        }

        private void NewPaymentSubmissionOp_Load(object sender, EventArgs e)
        {
            dgvPaymentRecords.AutoGenerateColumns = false;
            dgvFunding.AutoGenerateColumns = false;
            dgvKvpList.AutoGenerateColumns = false;
        ...
        private void dgvFunding_CellFormatting(object sender, DataGridViewCellFormattingEventArgs e)
        {
            try
            {
                if (dgvFunding.Rows[e.RowIndex].DataBoundItem != null)
                {
                    ...
                    switch (dgvFunding.Columns[e.ColumnIndex].DataPropertyName)
                    {
                        case "TotalAmount":
                            fd = (FundingDetail)dgvFunding.Rows[e.RowIndex].DataBoundItem;
                            e.Value = (decimal.Parse(fd.FFPAmount) + decimal.Parse(fd.SGFAmount)).ToString();
                            break;                        
                    }
                }
            }
        ...
        private void dgvPaymentRecords_SelectionChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                // populate funding list
                List<FundingDetail> fdList = pr.FundingDetailList;
                dgvFunding.DataSource = fdList;

                // populate kvp list
                var keyValuePairList = from row in pr.GenericNameValueList select new { Key = row.Key, Value = row.Value };
                dgvKvpList.DataSource = keyValuePairList.ToArray();
        ...
        private void ClearResponseFields(bool setResponseToNull = true)
        {
            ...
            {
                response = null;
                exportToolStripMenuItemReqAndResp.Enabled = false;
            }
            ...
            txtRespAcceptedRecCount.Text = string.Empty;
            txtRespRejectedRecCount.Text = string.Empty;

            this.ClearR1ResponseFields();
        }

        private void ClearR1ResponseFields()
        {
            // response rec1
            txtRespR1RecordID.Text = string.Empty;
            txtRespR1PaymentSetNumber.Text = string.Empty;
            txtRespR1Status.Text = string.Empty;
            txtRespR1StatusDate.Text = string.Empty;
            txtRespR1StatusMsg.Text = string.Empty;
        }
        ...
        private void LoadResponseFromObj(PaymentSubmissionResponse response)
        {
            this.ClearResponseFields(false);
            if (response == null) return;
            exportToolStripMenuItemReqAndResp.Enabled = true;
        ...
        private void LoadResponseRecordsFromObj(List<PaymentRecordStatus> prsList)
        {
            if (prsList == null) return;
            txtRespRecordCount.Text = prsList.Count.ToString();
            txtRespAcceptedRecCount.Text = prsList.FindAll(item => item.StatusCode == "Accepted").Count().ToString();
            txtRespRejectedRecCount.Text = prsList.FindAll(item => item.StatusCode == "RejectedFD").Count().ToString();

            string selectedRecNum = dgvPaymentRecords.SelectedRows[0].Cells["PymtRecNum"].Value.ToString();
            PaymentRecordStatus prs = prsList.Find(t => t.PaymentRecNumber == selectedRecNum);
            this.PopulateR1ResponseControlsFromObj(prs);
        }

        private void PopulateR1ResponseControlsFromObj(PaymentRecordStatus prs)
        {
            this.ClearR1ResponseFields();
            if (prs == null) return;
            txtRespR1RecordID.Text = prs.PaymentRecNumber + (string.IsNullOrWhiteSpace(prs.PaymentRecNumberExt) ? string.Empty : "_" + prs.PaymentRecNumberExt);
            txtRespR1PaymentSetNumber.Text = prs.PaymentSetNumber + (string.IsNullOrWhiteSpace(prs.PaymentSetNumberExt) ? string.Empty : "_" + prs.PaymentSetNumberExt); ;
            txtRespR1Status.Text = prs.StatusCode;
            txtRespR1StatusDate.Text = prs.StatusDate.ToString();
            txtRespR1StatusMsg.Text = prs.StatusNote;
        }
        ...
        private void btnShowPaymentDetails_Click(object sender, EventArgs e)
        {
            if (lblKvp.Visible)
            {
                btnShowPaymentDetails.Location = new Point(509, 738);

                dgvPaymentRecords.Height = 502;
                lblKvp.Visible = false;
                dgvKvpList.Visible = false;

                lblFundingList.Visible = false;
                dgvFunding.Visible = false;
                grpPymtResponse.Visible = false;
            }
            else
            {
                btnShowPaymentDetails.Location = new Point(509, 572);

                dgvPaymentRecords.Height = 334;
                lblKvp.Visible = true;
                dgvKvpList.Visible = true;

                lblFundingList.Visible = true;
                dgvFunding.Visible = true;
                grpPymtResponse.Visible = true;
            }            
        }
