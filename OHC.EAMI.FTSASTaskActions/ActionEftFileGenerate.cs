...
namespace OHC.EAMI.FTSASTaskActions
{
    ...
    [TaskAction(ActionName = "ActionEftFileGenerate", IsPhantom = false)]
    public class ActionEftFileGenerate : TaskActionBase
    {
        public override TaskResult Execute()
        {
            ...
            try
            {
                ...
                int paymentMethodTypeId = RefCodeDBMgr.GetRefCodeTableList().GetRefCodeListByTableName(enRefTables.TB_PAYMENT_METHOD_TYPE).Where(_ => _.Code == "EFT").First().ID;
        ...
        private List<IClaimpayment> PopulateSCOClaimPayment(ElectronicClaimSchedule ecs)
        {
            ...
            bool sco_HIPAA_Indicator = bool.Parse(RefCodeDBMgr.GetRefCodeTableList().GetRefCodeListByTableName(enRefTables.TB_SCO_PROPERTY).GetRefCodeByCode<SCOSetting>("SCO_HIPAA_INDICATOR").SCOSettingValue);
            EAMIMasterData eamiMasterData = new EAMIMasterData();
            //eamiMasterData.SystemProperty = new SystemProperty();

            eamiMasterData.SystemProperty = RefCodeDBMgr.GetRefCodeTableList().GetRefCodeListByTableName(enRefTables.TB_System).GetRefCodeByCode<SystemProperty>(RefCodeDBMgr.GetRefCodeTableList().GetRefCodeListByTableName(enRefTables.TB_SYSTEM_OF_RECORD)[0].Code);
            string federal_EIN_Number = eamiMasterData.SystemProperty.FEIN_Number.ToString();
            ...
            foreach (ClaimSchedule ecs_cs in ecs.ClaimScheduleList.OrderBy(_ => _.SeqNumber))
            {
                ...
                EamiClaimPayment eamiClaimPayment = new EamiClaimPayment()
                {
                    ...
                    VENDOR_CHECK_DIGIT = "",
                    VENDOR_DFI_ACCOUNT_NUMBER = cs.PayeeInfo.PEE_EftInfo.PrvAccountNo,
                    VENDOR_TRANSACTION_CODE = DetermineTransactionCode(cs.PayeeInfo.PEE_EftInfo.FIAccountType),
                    VENDOR_TRANSACTION_ROUTING_CODE = cs.PayeeInfo.PEE_EftInfo.FIRoutingNumber,
                    TRN02_Reference_ID = sco_HIPAA_Indicator ? cs.UniqueNumber : "",
                    TRN03_Company_ID = sco_HIPAA_Indicator ? federal_EIN_Number : ""
                };
        ...
        private List<ElectronicClaimSchedule> CreateSCOFile(List<ElectronicClaimSchedule> ecsList, string outputFolderPath)
        {
            ...
            foreach (ElectronicClaimSchedule ecs in ecsList)
            {
               ...
               // string scoDexFilePath = string.Empty;
                ...
                CommonStatusPayload<Tuple<string, int>> status = ScoServiceManager.CreateEFTFile(PopulateSCOClaimPayment(ecs), sco_HIPAA_Indicator);
        ...
        private string DetermineTransactionCode(string accountType)
        {
            string claimIdentifier = string.Empty;

            if (accountType.Trim() == "Checking")
            {
                return "22";
            }
            else
        ...
        private string ComposeElectronicClaimScheduleFileName(ElectronicClaimSchedule ecs, string scoFileName)
        {
            ...
            try
            {
                //Use Paydate in the file name
                dateStamp = ecs.PayDate;
                //Set the SCO filename
