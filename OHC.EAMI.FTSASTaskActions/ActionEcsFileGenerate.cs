...
namespace OHC.EAMI.FTSASTaskActions
{
    ...
    [TaskAction(ActionName= "ActionEcsFileGenerate", IsPhantom = false)] 
    public class ActionEcsFileGenerate : TaskActionBase
    {
        public override TaskResult Execute()
        {
            ...
            try
            {
                ...
                string localFolderPath = string.Format(@"{0}\{1}\", ConfigurationManager.AppSettings["RootPath"].ToString(), ConfigurationManager.AppSettings["folder_sco_send"].ToString());
                ...
                ecsList = DataAccess.ClaimScheduleDataDbMgr.GetElectronicClaimSchedulesByDateRangeStatusType(currentStatusTypeId, null, null);
                ...
                if (ecsList.Count() > 0)
                {
                    ...
                    //CREATE SCO FILES
                    List<ElectronicClaimSchedule> scoFileList = CreateSCOFile(ecsList, localFolderPath);
                                        
                    //UPDATE ESC status to DB
                    UpdateECSStatus(scoFileList);
                    
                    //ARCHIVE ECS FILE
                    ArchiveECSFiles(scoFileList, localFolderPath);
        ...
        private List<EamiClaimRemittanceAdviceRecord> PopulateRemittanceAdviceLines(ClaimSchedule cs, EamiClaimPayment cp)
        {
            ...
            cp.EamiClaimRemittanceAdviceRecordList.Add(AddSingleRimittanceAdviceLine(string.Format("{0}", DBSetting.GetDBSettingValue<string>(rcTableList, "RA_DEPARTMENT_NAME")).ToUpper(), Convert.ToDouble(cs.Amount)));
            cp.EamiClaimRemittanceAdviceRecordList.Add(AddSingleRimittanceAdviceLine(string.Format("{0}", DBSetting.GetDBSettingValue<string>(rcTableList, "RA_DEPARTMENT_ADDR_LINE")).ToUpper(), 0));
            cp.EamiClaimRemittanceAdviceRecordList.Add(AddSingleRimittanceAdviceLine(string.Format("{0}", DBSetting.GetDBSettingValue<string>(rcTableList, "RA_DEPARTMENT_ADDR_CSZ")).ToUpper(), 0));
            cp.EamiClaimRemittanceAdviceRecordList.Add(AddSingleRimittanceAdviceLine(string.Format("PAYMENT INQUIRIES: {0}", DBSetting.GetDBSettingValue<string>(rcTableList, "RA_INQUIRIES_PHONE_NUMBER")), 0));
        ...
        private void FilterExpiredClaimScheduleRecords(List<ClaimSchedule> claimScheduleList)
        {
            claimScheduleList.RemoveAll(_ => DateTime.Parse(_.PayDate.Code) < DateTime.Now.Date);
        }

        private void FilterFutureClaimScheduleRecords(List<ClaimSchedule> claimScheduleList, int leadDays)
        {
            claimScheduleList.RemoveAll(_ => DateTime.Parse(_.PayDate.Code) > DateTime.Now.AddDays(leadDays));
        }
        ...
        private List<ElectronicClaimSchedule> CreateSCOFile(List<ElectronicClaimSchedule> ecsList, string localFolderPath)
        {
            RefCodeTableList rcTableList = RefCodeDBMgr.GetRefCodeTableList();
            int orgID = DBSetting.GetDBSettingValue<int>(rcTableList, "RA_ORGANIZATION_CODE");
            ...
            foreach (ElectronicClaimSchedule ecs in ecsList)
            {
                ...
                string claimIdentifier = DetermineSCOClaimIdentifier(ecs, rcTableList);
                ScoServiceManager.CLAIM_IDENTIFIER = claimIdentifier;

                //CREATE CSO FILE STRING
                CommonStatusPayload<Tuple<string, int>> status = ScoServiceManager.CreateEFTFile(PopulateSCOClaimPayment(ecs));

                //CREATE CSO FILE
                if (status.Status)
                {
                    ecs.EcsFileName = ComposeElectronicClaimScheduleFileName(ecs);
                    
                    CommonStatus cs = CreateLocalFile(localFolderPath, ecs.EcsFileName, status.Payload.Item1);
        ...
        private List<ElectronicClaimSchedule> SendFilesToSCO(List<ElectronicClaimSchedule> ecsList, string localFolderPath)
        {
            RefCodeTableList rcTableList = RefCodeDBMgr.GetRefCodeTableList();
            RefCode claimScheduleStatusType_APPROVED = RefCodeDBMgr.GetRefCodeTableList().GetRefCodeListByTableName(enRefTables.TB_ECS_STATUS_TYPE).Where(_ => _.Code == "APPROVED").First();
            RefCode claimScheduleStatusType_SENT_TO_SCO = RefCodeDBMgr.GetRefCodeTableList().GetRefCodeListByTableName(enRefTables.TB_ECS_STATUS_TYPE).Where(_ => _.Code == "SENT_TO_SCO").First();
            RefCode claimScheduleStatusType_FAIL = RefCodeDBMgr.GetRefCodeTableList().GetRefCodeListByTableName(enRefTables.TB_ECS_STATUS_TYPE).Where(_ => _.Code == "FAIL").First();
            ...
            foreach (ElectronicClaimSchedule ecs in ecsList.Where(_ => _.CurrentStatusType.ID == claimScheduleStatusType_APPROVED.ID))
            {
                ...
                try
                {
                    ...
                    fileTransferManager.SFTP_Upload(new List<FileTransferInformation>() { new FileTransferInformation(localFolderPath, ecs.EcsFileName, scoUploadFolderName) });
                    cs.Status = true;
                }
                catch (Exception ex)
                {
                ...
                //SET STATUS
                ecs.CurrentStatusType = cs.Status ? claimScheduleStatusType_SENT_TO_SCO : claimScheduleStatusType_FAIL;
                ecs.CurrentStatusNote = cs.Status ? string.Empty : cs.GetFirstDetailMessage();
            }
            return ecsList;
        }

        private CommonStatus CreateLocalFile(string localUploadFolder, string fileName, string fileString)
        {
            ...
            {
                EAMILogger.Instance.Error(string.Format("Cannot create SCO file. File name: {0}\\{1}; {2}", fileName, ex.Message));
                EAMILogger.Instance.Error(ex);
                cs.Status = false;
        ...
        private string DetermineSCOClaimIdentifier(ElectronicClaimSchedule ecs, RefCodeTableList rcTableList)
        {
        ...
        private string ComposeElectronicClaimScheduleFileName(ElectronicClaimSchedule ecs)
        {
            ...
            if (ecs.ExclusivePaymentType.Code == "SCHIP")
            {
                dbSettingName = isProductionEnvironment ? "ECS_FILE_NAME_FUND_0555_PROD" : "ECS_FILE_NAME_FUND_0555_TEST";
            }
            else
            {
                dbSettingName = isProductionEnvironment ? "ECS_FILE_NAME_FUND_0912_PROD" : "ECS_FILE_NAME_FUND_0912_TEST";
            }
        ...
        private void ArchiveECSFiles(List<ElectronicClaimSchedule> scoFileList, string localFolderPath)
        {
            string archiveFolderPath = string.Format(@"{0}\{1}\", ConfigurationManager.AppSettings["RootPath"].ToString(), ConfigurationManager.AppSettings["folder_archive_ecs_to_sco"].ToString());

            foreach (ElectronicClaimSchedule scoFile in scoFileList)
            {
                string sourceFilePath = string.Format(@"{0}\{1}", localFolderPath, scoFile.EcsFileName);
