...
namespace OHC.EAMI.FTSASTaskActions
{
    ...
    [TaskAction(ActionName = "GenerateAndTransportECS_SIM")]
    public class ActionGenerateAndTransportECS_SIM : TaskActionBase
    {
        public override TaskResult Execute()
        {
            ...
            try
            {
                ...
                if (ecsList.Count() > 0)
                {
                    ...
                    List<ElectronicClaimSchedule> sentToSCOfileList = SendFilesToSCO(scoFileList, localFolderPath);

                    //UPDATE ESC status to DB
                    UpdateECSStatus(sentToSCOfileList);
        ...
        private List<ElectronicClaimSchedule> SendFilesToSCO(List<ElectronicClaimSchedule> ecsList, string localFolderPath)
        {
            ...
            foreach (ElectronicClaimSchedule ecs in ecsList.Where(_ => _.CurrentStatusType.ID == claimScheduleStatusType_APPROVED.ID))
            {
                ...
                try
                {
                    ...
                    ////cs = fileTransferManager.FTPS_Upload(string.Format(@"{0}{1}", localFolderPath, ecs.EcsFileName), ecs.EcsFileName);


                    //SIMULATE SUCCESSFULL SCO FILE UPLOAD
                    cs.Status = true;
                }
                catch (Exception ex)
                {
        ...
        private DataSet DetermineSCOClaimIdentifier(ElectronicClaimSchedule ecs, RefCodeTableList rcTableList)
        {
            ...
            try
            {
                dataSet = DataAccess.ClaimScheduleDataDbMgr.GetSCOFileProperty(ecsFundId, ecsSystemId, ecsPmtType, scoEnv, ecs.ExclusivePaymentType.IsActive);
            }
            catch (Exception ex)
            {
        ...
        private string ComposeElectronicClaimScheduleFileName(ElectronicClaimSchedule ecs, string scoFileName)
        {
            ...
            //DERRIVE A UNIQUE FILE INCREMENT BASED ON A SECOND OF TODAY's DATE
            System.Threading.Thread.Sleep(1000);
            DateTime today = DateTime.Today;
            fileNameIncrementValue = string.Format("{0:00000}", (int)dateStamp.Subtract(today).TotalSeconds);
        ...
        /* Eugene S 2018-11-13  this method is no longer used
        private CommonStatus InsertECStoDB(List<ElectronicClaimSchedule> ecsList)
        {
            CommonStatus cs = new CommonStatus(true);

            foreach (ElectronicClaimSchedule ecs in ecsList)
            {
                DataAccess.ClaimScheduleDataDbMgr.InsertElectronicClaimScheduleData(ecs);
            }
