...
{
    internal class ECSClaimSchedule : SCOClaimScheduleBase
    {
        #region Properties

        private ECSClaimHeader _scoClaimHeader;
        private List<Tuple<ECSPaymentDetail, ECSSecondaryPaymentDetail>> detailPayments;
        //private List<SCODetailPaymentStatement> detailStatementRecords;
        private ECSClaimTotalRecord claimTotal;
        private int _claimLineCount;

        public ECSClaimHeader ClaimHeader
        {
            get
            {
                if (_scoClaimHeader == null) _scoClaimHeader = new ECSClaimHeader();
                return _scoClaimHeader;
            }
        }

        public List<Tuple<ECSPaymentDetail, ECSSecondaryPaymentDetail>> PaymentDetails
        {
            get
            {
                if (detailPayments == null) detailPayments = new List<Tuple<ECSPaymentDetail, ECSSecondaryPaymentDetail>>();
                return detailPayments;
            }
        }

        public List<ECSDetailPaymentStatement> DetailStatementRecords
        {
            get
            {
                //if (detailStatementRecords == null) detailStatementRecords = new List<SCODetailPaymentStatement>();
                //return detailStatementRecords;
                return new List<ECSDetailPaymentStatement>();
            }
        }

        public ECSClaimTotalRecord ClaimTotalRecord
        {
            get
            {
                if (claimTotal == null) claimTotal = new ECSClaimTotalRecord();
                return claimTotal;
            }
        }
        ...
        public ECSClaimSchedule(string claimIdentifier, int claimNumber, int claimScheduleNumber, List<IClaimpayment> lstclaimPayments)
        {
            var claimPaymentList = lstclaimPayments.ToList();
            _scoClaimHeader = new ECSClaimHeader();
            detailPayments = new List<Tuple<ECSPaymentDetail, ECSSecondaryPaymentDetail>>();
            claimTotal = new ECSClaimTotalRecord();
            ...
            foreach (var claimPayment in claimPaymentList.OrderBy(_ => _.SEQ_NUMBER))
            {
                ...
                ECSPaymentDetail scoPaymentDetail = new ECSPaymentDetail();
                ...
                ECSSecondaryPaymentDetail sscoPaymentDetail = new ECSSecondaryPaymentDetail();
                ...
                {
                    scoRALineNumber++;
                    ECSDetailPaymentStatement scoDetailPaymentStatement = new ECSDetailPaymentStatement();
                ...
                {
                    scoRALineNumber++;
                    ECSAuditDetail wrAuditLine = new ECSAuditDetail();
                ...
                detailPayments.Add(new Tuple<ECSPaymentDetail, ECSSecondaryPaymentDetail>(scoPaymentDetail, sscoPaymentDetail));
                _claimLineCount++;
            }
