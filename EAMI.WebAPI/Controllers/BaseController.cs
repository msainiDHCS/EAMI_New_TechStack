...
namespace EAMI.Controllers
{
   ...
   // [AuthorizeResource(EAMIRole.ADMINISTRATOR, EAMIRole.PROCESSOR, EAMIRole.SUPERVISOR)]
    public class BaseController : Controller
    {
        ...
        #region Commenting this code for now...
        /*        
        [HttpGet("GetReferenceCodes")]
        ...
        public int GetLoggedinUserID()
        {
            ...
            {
               // var uidentity = user;
                if (user.HasClaim(a => a.Type == "UserID"))
                {
                    int.TryParse(user.FindFirst("UserID")?.Value, out uid);
                }
            }
        ...
        /// <returns></returns>
        //[AjaxOnly]
        [HttpGet("DisplayGroupFundingDetails")]
        public IActionResult DisplayGroupFundingDetails(List<PaymentSuperGroup> psgs, int paymentRecordID, string topGroupID, string parentPaymentRecordSetNumber)
        {
        ...
        /// <returns></returns>
        //[AjaxOnly]
        [HttpGet("DisplayCSFundingDetails")]
        public IActionResult DisplayCSFundingDetails(List<ClaimSchedule> csList, string csUniqueNumber, int paymentRecordID, string parentPaymentRecordSetNumber)
        {
            IPFundingDetails fds = new IPFundingDetails();
        ...
        private IPFundingDetails FillupFundingDetails(List<PaymentSuperGroup> psgs, int paymentRecordID, string topGroupID, string ParentPaymentRecordSetNumber)
        {
            ...
            PaymentRec pr = psgs.FirstOrDefault(a => a.UniqueKey == topGroupID).PaymentGroupList.Where(a => a.UniqueNumber == ParentPaymentRecordSetNumber).FirstOrDefault()
